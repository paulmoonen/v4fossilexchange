<template>
	<div class="col-4">
		<form class="productcard " @submit.prevent="addToBasket">
			...
			<img :src="this.getImgUrl" alt="image" class="img-fluid" v-show="(this.image)"> <!-- only show an image element if there is an image -->
			<span v-show="(!this.image)">No image available &#128248; </span>
			<button type="submit" :disabled="!this.stockcounter">&#128722; add to cart</button>
...
export default {
	...
	mounted:function(){
		...
		this.eventbus.$on('emptycart', () => {this.reset()});
